            ...
            <div className={`grid ${embedded ? "gap-4" : "gap-6"}`}>
                {config.items.map((item, index) => (
                    <motion.div
                        ...
                        className={`overflow-hidden bg-white dark:bg-neutral-900 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-800 hover:shadow-lg transition-all duration-200 hover:scale-[1.01]`}
                    >
                        {item.image && (
                            <div className="relative aspect-[16/9] bg-neutral-100 dark:bg-neutral-800">
                                <Image
                                    ...
                                    sizes="(max-width: 768px) 100vw, (max-width: 1200px) 70vw, 768px"
                                />
                            </div>
                        )}
                        <div className={embedded ? "p-4" : "p-6"}>
                            <div className="flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-start mb-2">
                                ...
                                {item.date && (
                                    <span className="text-sm text-neutral-500 font-medium bg-neutral-100 dark:bg-neutral-800 px-2 py-1 rounded self-start">
                                        {item.date}
                                    </span>
                                )}
                            </div>
                                ...
                                <p className={`${embedded ? "text-sm" : "text-base"} text-accent font-medium mb-3`}>
                                    {item.subtitle_link ? (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="rounded transition-colors hover:text-accent-dark"
                                        >
                                            {item.subtitle}
                                        </a>
                                    ) : (
                            ...
                            {item.link && (
                                <a
                                    ...
                                    className="inline-flex mt-4 items-center justify-center px-3 py-2 rounded-md text-sm font-medium bg-primary text-background hover:bg-accent transition-colors"
                                >
                                    Open
                                </a>
                            )}
