...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    // Determine type. By default this follows the BibTeX entry type, but a
    // custom publication_type field can override it for edge cases.
    const explicitType = normalizePublicationType(tags.publication_type || tags.pubtype);
    const type = explicitType || typeMapping[entry.entryType.toLowerCase()] || 'journal';
    ...
    const publication: Publication = {
      ...
      posterUrl: tags.poster || tags.posterurl,
      videoUrl: tags.video || tags.videourl,
      ...
      bibtex: reconstructBibTeX(entry, ['selected', 'preview', 'description', 'keywords', 'code', 'pdf', 'pdfurl', 'poster', 'posterurl', 'video', 'videourl', 'publication_type', 'pubtype']),
    };
...
function normalizePublicationType(value?: string): PublicationType | null {
  if (!value) return null;

  const normalized = cleanBibTeXString(value).toLowerCase().replace(/[\s_]+/g, '-');
  ...
  return validTypes.includes(normalized as PublicationType) ? normalized as PublicationType : null;
}
